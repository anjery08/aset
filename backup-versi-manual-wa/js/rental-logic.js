...
function ambilHeaderAdminAuth() {
    ...
    try {
        ...
        if (!savedToken) {
            ...
            savedToken = config.password || "metahati2026";
        }
        if (savedToken) {
...
function simpanPengaturanAdmin(config) {
    try {
        ...
        try {
            fetch(dapatkanApiEndpoint('action=simpan_pengaturan'), {
                method: 'POST',
...
function tambahKategoriBaru(namaLabel) {
    ...
    try {
        ...
        fetch(dapatkanApiEndpoint('action=simpan_kategori'), {
            method: 'POST',
            headers: ambilHeaderAdminAuth(),
...
function simpanAsetKustom(asetData) {
    try {
        ...
        try {
            fetch(dapatkanApiEndpoint('action=simpan_aset'), {
                method: 'POST',
                headers: ambilHeaderAdminAuth(),
...
async function tolakBookingAdminAsync(bookingId, alasan = 'Unit tidak dapat disewakan pada jam tersebut') {
    ...
    try {
        const res = await fetch(dapatkanApiEndpoint('action=tolak_booking'), {
            method: 'POST',
            headers: ambilHeaderAdminAuth(),
...
async function accBookingAdminAsync(bookingId, opsi = {}) {
    ...
    // 2. Kirim ke server api.php
    try {
        const res = await fetch(dapatkanApiEndpoint('action=acc_booking'), {
            method: 'POST',
            headers: ambilHeaderAdminAuth(),
            body: JSON.stringify({
                bookingId: bIdClean,
...
async function kembalikanBookingAdminAsync(bookingId, returnData = {}) {
    ...
    try {
        ...
        const res = await fetch(dapatkanApiEndpoint('action=kembalikan_booking'), {
            method: 'POST',
            headers: ambilHeaderAdminAuth(),
...
async function hapusBookingServer(bookingId) {
    try {
        ...
        await fetch(dapatkanApiEndpoint('action=hapus_booking'), {
            method: 'POST',
            headers: ambilHeaderAdminAuth(),
...
async function kirimNotifikasiBookingOtomatis(booking) {
    ...
    if (adminPhone && gw.notifyAdminNewBooking !== false) {
        const pesanAdmin = `🔔 *PERMOHONAN SEWA / PINJAM ASET BARU*\n\n` +
            ...
            `_Notifikasi otomatis Bot Gateway Inventaris Ma'had Aly Amtsilati._`;
